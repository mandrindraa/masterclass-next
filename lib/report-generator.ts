...
export const generateReportCard = async (student: Student, period: string) => {
  ...
  doc.text(`Academic Period: ${period}`, pageWidth / 2, 55, {
    align: "center",
  });
  ...
  doc.text(
    "Excellence Academy - Academic Report",
    pageWidth / 2,
    pageHeight - 20,
    { align: "center" }
  );
  ...
  // Save the PDF
  doc.save(
    `${student.name.replace(/\s+/g, "_")}_Report_Card_${period.replace(
      /\s+/g,
      "_"
    )}.pdf`
  );
...
export const generateBulkReports = async (
  students: Student[],
  period: string
) => {
...
export const exportStudentGradesToCSV = (
  students: Student[],
  period: string
) => {
  ...
  link.setAttribute(
    "download",
    `Student_Grades_${period.replace(/\s+/g, "_")}.csv`
  );
